...
% ovl(ovl==1)=nan;
% ovl(ovl==0)=1;
vradLegIn.v1(vradLegIn.ovl==1)=-999;
...
s1=nexttile(1);
hold on
h2=surf(XX(:,1:end-1),YY(:,1:end-1),flipud(lev2.VEL),'edgecolor','none');
view(2);
...
applyColorScale(h2,flipud(lev2.VEL),vel_default3,colLims2);
...
h1=surf(XX(:,1:end-1),YY(:,1:end-1),flipud(vradLeg.VEL),'edgecolor','none');
view(2);
...
print([figdir,radar,'.png'],'-dpng','-r0');
